...
def train(rank, a, h):
    ...
    trainset = Dataset(training_indexes, a.input_clean_wavs_dir, a.input_noisy_wavs_dir,
                       ...
                       split=True, n_cache_reuse=0, shuffle=False if h.num_gpus > 1 else True, device=device,indexes_clean=training_indexes_clean)
    ...
    if rank == 0:
        validset = Dataset(validation_indexes, a.input_clean_wavs_dir, a.input_noisy_wavs_dir,
                           h.segment_size, h.n_fft, h.hop_size, h.win_size, h.sampling_rate, h.compress_factor,
                           split=True, shuffle=False, n_cache_reuse=0, device=device,indexes_clean=validation_indexes_clean)
...
def main():
    ...
    parser.add_argument('--input_clean_wavs_dir', default='/sda1/lanxin/MP-SENet-main/VoiceBank_DEMAND/wavs_clean/clean')
    parser.add_argument('--input_noisy_wavs_dir', default='/sda1/lanxin/MP-SENet-main/VoiceBank_DEMAND/wavs_noisy/noisy')
    parser.add_argument('--input_training_file', default='VoiceBank_DEMAND/training.txt')
    parser.add_argument('--input_validation_file', default='VoiceBank_DEMAND/test.txt')
    parser.add_argument('--checkpoint_path', default='cp_mpsenet')
    parser.add_argument('--config', default='/sda1/lanxin/MP-SENet-main/config.json')

...
def inference(a):
    ...
    with torch.no_grad():
        for i, index in enumerate(test_indexes):
            ...
            audio_start = 0
            # print(h.sampling_rate) #16k
            noisy_wav = torch.FloatTensor(noisy_wav)
            enhance_wav = torch.zeros_like(noisy_wav)
            while (noisy_wav.size(0) - audio_start) > 0: #>= a.segment_size: 
                ...
                noisy_amp, noisy_pha, noisy_com = mag_pha_stft(noisy_audio, h.n_fft, h.hop_size, h.win_size, h.compress_factor)
...
def main():
    ...
    parser.add_argument('--input_noisy_wavs_dir', default='VoiceBank_DEMAND/TEST/B')

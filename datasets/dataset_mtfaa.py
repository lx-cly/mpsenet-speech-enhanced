...
def get_dataset_filelist(a):
    with open(a.input_training_file, 'r', encoding='utf-8') as fi:
        lines = fi.readlines() 
        training_indexes = [x.split('|')[0] for x in lines if len(x) > 0] #[x.split('|')[0] for x in fi.read().split('\n') if len(x) > 0]
        training_indexes_clean = [x.split('|')[1].split('\n')[0] for x in lines if len(x) > 0]
    with open(a.input_validation_file, 'r', encoding='utf-8') as fi:
        lines = fi.readlines()
        validation_indexes = [x.split('|')[0] for x in lines if len(x) > 0]
        validation_indexes_clean = [x.split('|')[1].split('\n')[0] for x in lines if len(x) > 0]
    return training_indexes, validation_indexes,training_indexes_clean,validation_indexes_clean


class Dataset(torch.utils.data.Dataset):
    def __init__(self, training_indexes, clean_wavs_dir, noisy_wavs_dir, segment_size, n_fft, hop_size, win_size, 
                sampling_rate, compress_factor, split=True, shuffle=True, n_cache_reuse=1, device=None,indexes_clean=None):
        self.audio_indexes = training_indexes
        self.audio_indexes_clean = indexes_clean
        # random.seed(1234)
        # if shuffle:
    ...
    def __getitem__(self, index):
        ...
        if self._cache_ref_count == 0:
            clean_filename = self.audio_indexes_clean[index]  #'clean'+'_'+filename.split('_')[1]+'_'+filename.split('_')[2] #clean
            clean_audio, _ = librosa.load(clean_filename, sr=self.sampling_rate)
            noisy_audio, _ = librosa.load(filename, sr=self.sampling_rate)
            # clean_filename = 'clean'+'_'+filename.split('_')[1]+'_'+filename.split('_')[2] #clean
            # clean_audio, _ = librosa.load(os.path.join(self.clean_wavs_dir, clean_filename + '.wav'), sr=self.sampling_rate)
        ...
        return (clean_audio.squeeze(),noisy_audio.squeeze())#(clean_audio.squeeze(), clean_mag.squeeze(), clean_pha.squeeze(), clean_com.squeeze(), noisy_mag.squeeze(), noisy_pha.squeeze())

    def __len__(self):
        return len(self.audio_indexes)

    def collate_fn(self, batch): #为了防止每轮都裁剪不同音频
        random.seed(7349)
        return batch